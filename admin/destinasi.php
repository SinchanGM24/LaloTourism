...
if ($result_admin->num_rows > 0) {
    ...
    if ($op == 'delete') {
        if (isset($_GET['id_object'])) {
            $id = $_GET['id_object'];
            $sql2 = "DELETE FROM objek_wisata WHERE id_object = '$id'";
            ...
            $error = "ID objek wisata tidak ditemukan";
        }
    }

    if ($op == 'edit' && isset($_GET['id_object'])) {
        $id = $_GET['id_object'];
        $sql2 = "SELECT * FROM objek_wisata WHERE id_object = '$id'";
        $q2 = $koneksi->query($sql2);
        if ($q2 && $q2->num_rows > 0) {
            $r1 = $q2->fetch_assoc();
            $nama = $r1['nama_objek'];
            $deskripsi = $r1['deskripsi_objek'];
            $media = $r1['media_objek'];
        } else {
    ...
    if (isset($_POST['simpan'])) { //untuk create
        $nama       = $_POST['nama_objek'];
        $deskripsi  = $_POST['deskripsi_objek'];
    
        // Process file upload
        if (isset($_FILES['media_objek']) && $_FILES['media_objek']['error'] == 0) {
            $targetDir = "../asset/"; // Tentukan direktori penyimpanan file
            $fileName = basename($_FILES["media_objek"]["name"]);
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["media_objek"]["tmp_name"], $targetFilePath)) {
        ...
        if ( $nama && $deskripsi && $media) {
            if ($op == 'edit') { //untuk update
                $sql2       = "UPDATE objek_wisata set nama_objek='$nama', deskripsi_objek = '$deskripsi', media_objek='$media' where id_object = '$id'";
                $q2 = $koneksi->query($sql2);
                if ($q2) {
                ...
                $sql2   = "INSERT INTO objek_wisata(nama_objek,deskripsi_objek,media_objek) values ('$nama','$deskripsi','$media')";
            ...
            <li class="nav-item active">
                ...
                <div id="collapseUtilities" class="collapse show" aria-labelledby="headingUtilities"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                        <a class="collapse-item active" href="destinasi.php">Destinasi</a>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="mx-auto">
                        <a href="NambahDestinasi.php"><button type="button" class="tambahkan">Tambahkan</button></a>
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            ...
                                            while ($r2 = $q2->fetch_assoc()) {
                                                $id = $r2['id_object'];
                                                $nama = $r2['nama_objek'];
                                                $deskripsi = $r2['deskripsi_objek'];
                                                $media = $r2['media_objek'];
                                            ?>
                                                <tr>
                                                    <td><?php echo $urut++ ?></td>
                                                    <td><?php echo $nama ?></td>
                                                    ...
                                                    <td>
                                                        <a href="destinasi.php?op=edit&id_object=<?php echo $id ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                                                        <a href="destinasi.php?op=delete&id_object=<?php echo $id ?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>
                                                    </td>
                                                </tr>
                                            <?php
                                            }
                        ...
                        <div class="card-body ">
                            ...
                            <form action="" method="POST" enctype="multipart/form-data">
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="nama" name="nama_objek" value="<?php echo $nama ?>">
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="deskripsi" class="col-sm-2 col-form-label">Deskripsi</label>
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" id="deskripsi" name="deskripsi_objek" value="<?php echo $deskripsi ?>">
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="media" class="col-sm-2 col-form-label">Media</label>
                                    <div class="col-sm-10">
                                        <input type="file" class="form-control" id="media" name="media_objek" value="<?php echo $media ?>" >
                                    </div>
                                </div>
